...
namespace {
	...
	void print_collision_groups(std::uint64_t flags) {
		std::map<std::string, COL_LAYER> named_layers {
			...
			{ "kCameraPick", COL_LAYER::kCameraPick },
			{ "kItemPick", COL_LAYER::kItemPick },
			{ "kLOS", COL_LAYER::kLOS },
			{ "kPathPick", COL_LAYER::kPathPick },
...
namespace GTS {
	...
	void ContactListener::sync_camera_collision_groups() {
		...
		auto& cameraBitfield = filter->layerBitfields[static_cast<uint8_t>(COL_LAYER::kCameraPick)];
